...
| Apertura ≤2 min; fotos sólo por excepción; fondo de caja al abrir | ✅ | Foto en apertura sólo por muestreo determinístico (`photo_sampling_pct`, `require_open_photo`); AYUDA "otro" con foto |
...
## specs/pos-cash
Ledger con folio/actor/timestamp/contexto ✅ · append-only ✅ (cancelación/devolución como registro nuevo; `cash_movements` con actor y motivo) · conciliación POS × efectivo × digital × inventario ✅ (`services/cash`) · cierre conciliado / diferencia material → caso sin tocar ledger ✅ · cancelaciones/descuentos con permiso y motivo ✅ (descuentos: ⬜ no existen en el catálogo MVP; devoluciones ✅ `reason_code=return` con caso de revisión). Efectivo esperado = fondo + efectivo − retiros/gastos/devoluciones ✅ (`services/cash.py`, `offline/expected.ts`).

## specs/inventory
Inventario central/por punto/en tránsito 🟡 (por punto y movimientos completos; almacén central y "en tránsito" existen como tablas `warehouses`/tipos `transfer_*` pero sin UI de almacén) · operador sin SKU/lote ✅ (recepción y conteo también en la PWA: `/recibir`, `/contar`) · consumo teórico vs conteo ✅ · reposición por reglas (`stock_critical`) ✅.
...
## specs/supervisor
Cola URGENTE/REVISAR/NORMAL ✅ · ruta sugerida por severidad/impacto/antigüedad/distancia ✅ (`supervisor/route`, vecino más cercano) · muestreo de puntos normales ✅ (`route_sampling_normal_pct`, determinístico por día) · auditoría Sí/No con evidencia opcional y notas por voz ✅ · correctivos con responsable/fecha/estado ✅ · seguimiento automático ✅ (SLA por severidad con escalado; notificaciones push/WhatsApp con bitácora; `overdue` en acciones).
...
| PDF servidor programado | ✅ | `services/scheduled_reports.py` (job 07:05, HTML; PDF si WeasyPrint) · CSV `export.csv` · `POST /send` |
| Agregados diarios | ⬜ | fase 2 (`docs/REPORTES.md` §7) |

## specs/security · security-ai-governance
RBAC ✅ (incluye `reports.*`) · auditoría antes/después ✅ · auditoría de consultas de reportes ✅ (`report.view`/`report.export`) · MFA ✅ (TOTP para admin/finanzas, `mfa_enforce`) · GPS con política de retención ✅ (`run_maintenance`: purga de gps_pings y evidencias) · GPS simulado como señal ✅ (`gps_pings.mocked`, regla no sanciona) · segregación de funciones ✅ · dispositivo perdido ✅ · TLS ✅ (Caddy con cabeceras HSTS/CSP; CA interna en LAN) · rate limiting ✅ (`RateLimitMiddleware` por IP + límite de login) · LLM no escribe ledgers ✅ · versionado de modelo ✅ (`model_version`) · evaluación/drift ⬜.
