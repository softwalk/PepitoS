...
| GET | `/v1/people/ranking?period=day\|month\|year` | Ranking de vendedores por ventas `recorded` en el periodo local (1 = mejor, empates comparten lugar; supervisor sólo su zona). Se guarda en `users.sales_rank_{day,month,year}` / `sales_{period}_cents` / `sales_rank_at`; lo recalcula el motor cada corrida, cada cierre y `POST /v1/rules/run`. `GET /v1/me/assignment` devuelve `ranking` para el operador; `GET /v1/admin/users` incluye `ranking` en operadores |
| — | Nombre de punto | En todas las respuestas el `name` de un punto es `Point.display_name` = «Nombre - Score» cuando el punto tiene score estratégico (`meta.score` del catálogo); `GET /v1/admin/points` expone además `name` crudo, `display_name` y `score` |
